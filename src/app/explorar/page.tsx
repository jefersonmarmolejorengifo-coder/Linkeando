...
function Stars({ rating }: { rating: number }) {
  ...
  return <span className="text-[#EF9F27] text-[11px]">{'★'.repeat(full)}{'☆'.repeat(5 - full)} {rating.toFixed(1)}</span>
}
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f3] flex justify-center">
      <div className="w-full max-w-sm flex flex-col min-h-screen">
        {/* Header */}
        <div className="bg-[#1D9E75] px-4 pt-4 pb-5">
        ...
        <div className="px-4 pt-3 space-y-2">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
            <button
              ...
              className={`flex-shrink-0 px-3 py-1.5 rounded-full text-[11px] border ${!catFiltro ? 'bg-[#1D9E75] text-white border-[#1D9E75]' : 'bg-white text-gray-500 border-gray-200'}`}
            >
              Todas
            </button>
            {CATEGORIAS.map((cat) => (
              <button
                ...
                className={`flex-shrink-0 px-3 py-1.5 rounded-full text-[11px] border ${catFiltro === cat.key ? 'bg-[#1D9E75] text-white border-[#1D9E75]' : 'bg-white text-gray-500 border-gray-200'}`}
              >
                {cat.icon} {cat.label}
              </button>
            ))}
          ...
          <select
            ...
            className="w-full border border-gray-200 rounded-xl px-3 py-2 text-[12px] bg-white focus:outline-none focus:border-[#1D9E75]"
          >
              ...
              {filtrados.map((pro) => {
                ...
                return (
                  <button
                    ...
                    className="w-full bg-white rounded-xl border border-[#e8e8e6] px-3 py-3 flex gap-3 items-center text-left hover:border-[#1D9E75] transition-colors"
                  >
                    <div
                    ...
                    <div className="text-right flex-shrink-0">
                      {pro.tarifa && <p className="text-[12px] font-medium text-[#1D9E75]">${(pro.tarifa / 1000).toFixed(0)}k/h</p>}
        ...
        <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-sm bg-white border-t border-[#e8e8e6] flex justify-around py-2 z-40">
          {[
            ...
            <button key={item.label} onClick={() => router.push(item.href)} className={`flex flex-col items-center gap-0.5 text-[9px] px-2 border-none bg-transparent ${item.active ? 'text-[#1D9E75]' : 'text-gray-400 hover:text-gray-600'}`}>
              {item.icon}
